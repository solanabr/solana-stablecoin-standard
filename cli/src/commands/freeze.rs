...
pub fn execute(config: &CliConfig, args: &FreezeArgs) -> Result<()> {
    let (config_pda, _) = get_config_pda(&args.mint);
    let (role_registry_pda, _) = get_role_registry_pda(&config_pda);

    let accounts = sss_token::accounts::FreezeTokenAccount {
    ...
    let ix_data = sss_token::instruction::FreezeAccount {}.data();
    ...
    println!("Account frozen. Signature: {}", sig);
