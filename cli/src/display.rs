...
use sss_token::state::{
    BlacklistEntry, MinterInfo, ReserveAttestation, RoleRegistry, StablecoinConfig,
};
...
pub fn display_config(config: &StablecoinConfig, roles: &RoleRegistry) {
    ...
    if config.enable_permanent_delegate {
        features.push("permanent_delegate");
    }
    if config.enable_transfer_hook {
        features.push("transfer_hook");
    }
    if config.default_account_frozen {
        features.push("default_frozen");
    }
    if config.enable_confidential_transfers {
        features.push("confidential_transfers");
    }
    ...
    println!(
        "  {} {}",
        "Master Authority:".bold(),
        roles.master_authority
    );
    println!(
        "  {} {}",
        "Pauser:".bold(),
        format_optional_role(&roles.pauser)
    );
    println!(
        "  {} {}",
        "Blacklister:".bold(),
        format_optional_role(&roles.blacklister)
    );
    println!(
        "  {} {}",
        "Seizer:".bold(),
        format_optional_role(&roles.seizer)
    );
    println!();
}
...
pub fn display_minter(minter: &MinterInfo) {
    ...
    println!(
        "  {} {}",
        "Quota:".bold(),
        if minter.mint_quota == 0 {
            "Unlimited".to_string()
        } else {
            minter.mint_quota.to_string()
        }
    );
...
pub fn display_attestation(attestation: &ReserveAttestation) {
    let hash_hex: String = attestation
        .reserve_hash
        .iter()
        .map(|b| format!("{:02x}", b))
        .collect();
    ...
    println!(
        "  {} {} cents",
        "Total Reserves (USD):".bold(),
        attestation.total_reserves_usd
    );
    println!(
        "  {} {}",
        "Total Outstanding:".bold(),
        attestation.total_outstanding
    );
