 ...
 */
function parseEvent(logs, signature) {
    const events = [];
...
// ==================== EVENT HANDLERS ====================
async function processStablecoinInitialized(data) {
...
async function processTokensMinted(data) {
    await supabase.from("events").insert({
...
async function processTokensBurned(data) {
    await supabase.from("events").insert({
...
async function processAccountFrozen(data) {
    await supabase.from("events").insert({
...
async function processAccountThawed(data) {
    await supabase.from("events").insert({
...
async function processStablecoinPaused(data) {
    await supabase.from("events").insert({
...
async function processStablecoinUnpaused(data) {
    await supabase.from("events").insert({
...
async function processRolesUpdated(data) {
    await supabase.from("events").insert({
...
async function processMinterQuotaUpdated(data) {
    await supabase.from("events").insert({
...
async function processAuthorityTransferred(data) {
    await supabase.from("events").insert({
...
async function processBatchMinted(data) {
    await supabase.from("events").insert({
...
async function processTransferExecuted(data) {
    await supabase.from("events").insert({
...
async function processBlacklistAdded(data) {
    await supabase.from("events").insert({
...
async function processBlacklistRemoved(data) {
    await supabase.from("events").insert({
...
async function processTokensSeized(data) {
    await supabase.from("events").insert({
...
async function processConfigUpdated(data) {
    await supabase.from("events").insert({
...
// ==================== HELPERS ====================
async function updateHolderBalance(owner, stablecoin, amount, operation) {
...
async function indexEvents() {
    try {
        ...
        const signatures = await connection.getSignaturesForAddress(sss_token_1.SSS_TOKEN_PROGRAM_ID, { limit: 10 }, "confirmed");
        for (const sigInfo of signatures) {
            if (sigInfo.slot <= lastProcessedSlot)
                continue;
        ...
        const hookSignatures = await connection.getSignaturesForAddress(sss_token_1.SSS_TRANSFER_HOOK_PROGRAM_ID, { limit: 10 }, "confirmed");
