...
pub fn process_mint(ctx: Context<MintToken>, mut amount: u64) -> Result<()> {
    ...
    if let Some(feed_pubkey) = config.oracle_feed {
        ...
        let multiplier = 10u64
            .checked_pow(oracle_decimals as u32)
            .ok_or(StablecoinError::MathOverflow)?;
        let adjusted = (amount as u128)
            .checked_mul(multiplier as u128)
            .ok_or(StablecoinError::MathOverflow)?
            .checked_div(price as u128)
            .ok_or(StablecoinError::InvalidOraclePrice)?;
        amount = u64::try_from(adjusted).map_err(|_| error!(StablecoinError::MathOverflow))?;
