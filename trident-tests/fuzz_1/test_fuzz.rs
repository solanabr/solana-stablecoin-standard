...
impl FuzzTest {
    ...
    #[flow]
    fn pause_after_giving_role(&mut self) {
        let admin = self.fuzz_accounts.admin.get(&mut self.trident).unwrap();
        ...
        let (pause_authority, _) = pubkey::Pubkey::find_program_address(
            &[FuzzTest::PAUSE_SEED, mint.as_ref()],
            &sss::program_id(),
        );

        self.trident.process_transaction(
            &[
                sss::PauseInstruction::data(sss::PauseInstructionData::new())
                    .accounts(sss::PauseInstructionAccounts::new(
                        admin,
                        mint,
                        pauser_role,
                        pause_authority,
            ...
            Some("Allow pauser to pause after role is granted"),
        );
    }
